...
- anon キーはクライアントに埋め込まれ**公開される**。アプリは Cloudflare Pages 上にあるが、**クライアントは `*.supabase.co` を直接叩くため Cloudflare の WAF / レート制限は前段に入らない**
- そのため、スクレイピングや濫用による無料枠の消費、`nearby_shelters` のような計算重めの RPC が無防備な公開エンドポイントになり得る点に注意する
- PoC では Supabase 組み込みのレート制限・上限の範囲で運用し、濫用対策が本格的に必要になった時点で中間 API レイヤー（B）を前段に置いて Cloudflare で制御する（§5 の移行トリガー参照）
...
- **RPC は `SECURITY INVOKER`（デフォルト）で定義し、anon に `GRANT EXECUTE` する**こと。`SECURITY DEFINER` で書くと定義者（高権限ロール）の権限で実行され **RLS をバイパスする**ため、「RLS で読み取り制御が完結する」という本設計の前提が崩れる。空間検索 RPC は公開データの参照のみであり、RLS を効かせたまま実行する必要がある（具体的な関数定義は別 Issue）
...
- 公開 anon アクセスへの**濫用対策・レート制限**を前段で行う必要が出たとき
...
- 近傍検索用の PostGIS RPC 関数（`nearby_shelters` 等）の定義は別 Issue で追加する。その際は **`SECURITY INVOKER` で定義し anon に `GRANT EXECUTE`**（RLS をバイパスしないこと）
- クライアントのデータ取得は 1 モジュールに集約し、中間 API レイヤー（B）への差し替えコストを抑える
- 中間 API を導入する場合は Cloudflare（Pages Functions / Workers）で実装し、基盤を分断しない
- 高頻度データ（警報は 5 分更新）は多クライアントが直接 PostgREST を叩くと都度 DB ヒットになる。スケール時は GET レスポンスの HTTP キャッシュ活用や中間 API でのキャッシュを検討する
